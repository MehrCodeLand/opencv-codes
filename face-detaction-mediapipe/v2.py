...
# Function to analyze blinking patterns
def analyze_blinking(blink_frames, threshold=10):
    if blink_frames > threshold:
        return False  # If eyes stay closed/open for too long, it's likely fake
    return True
    ...
    while cap.isOpened():
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                # Check if the object is blinking appropriately
                if frame_count > 30:  # Analyze after initial frames
                    if not analyze_blinking(closed_eye_frames, threshold=20):
                        cv2.putText(frame, "Fake Detected: Static Eyes", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    else:
                ...
                # Additional anti-spoofing check (head movement analysis)
                dx = abs(face_landmarks.landmark[33].x - face_landmarks.landmark[263].x)
                if dx < 0.001:  # Minimal change in head pose over frames
                    cv2.putText(frame, "Fake Detected: No Head Movement", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

        # Display the results
        if blink_detected and real_face_detected:
            cv2.putText(frame, "Live Face Detected", (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
