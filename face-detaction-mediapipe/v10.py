...
def detect_mask(face_landmarks, frame, frame_width, frame_height):
    ...
    nose = (int(face_landmarks[1][0]), int(face_landmarks[1][1]))  # Nose tip
    chin = (int(face_landmarks[152][0]), int(face_landmarks[152][1]))  # Chin
    left_cheek = (int(face_landmarks[234][0]), int(face_landmarks[234][1]))  # Left cheek
    right_cheek = (int(face_landmarks[454][0]), int(face_landmarks[454][1]))  # Right cheek

    mask_region = frame[nose[1]:chin[1], left_cheek[0]:right_cheek[0]]
    if mask_region.size == 0:
        return False

    # Analyze the region for mask-like characteristics (low variability in color)
    gray = cv2.cvtColor(mask_region, cv2.COLOR_BGR2GRAY)
    std_dev = np.std(gray)  # Standard deviation of intensity
